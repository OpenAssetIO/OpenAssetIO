...
Asset-managed pipelines do not conform to a single structure — each
...
functionality was begun but not proven out, and the approach was not
...
as a generic interface, the methods were not application-specific, being
...
UI elements. The UI delegate is provided with its corresponding
`Manager` instance on construction, thus avoiding the need for
singletons (FnAssetAPI-style).
...
- The UI delegate will need access to the core manager plugin. This will
  be limited to the public `Manager` object (like any other consumer),
  unless the architecture of the API/plugin system is modified.
...
- Tight coupling of UI and headless concerns means care must be taken,
  ...
  are completely headless and do not require UI functionality.
